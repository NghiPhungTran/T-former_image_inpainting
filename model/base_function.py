...
def init_net(net, init_type='normal', gpu_ids=None):
    """In cấu trúc mạng, khởi tạo trọng số và chuyển mạng sang GPU (nếu có)"""
    if gpu_ids is None:
        gpu_ids = []
    ...
    if torch.cuda.is_available() and len(gpu_ids) > 0:
        net.to(torch.device(f"cuda:{gpu_ids[0]}"))  # chuyển mạng lên GPU đầu tiên
        net = torch.nn.DataParallel(net, device_ids=gpu_ids)  # hỗ trợ multi-GPU
