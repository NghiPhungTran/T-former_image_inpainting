...
class CreateDataset(data.Dataset):
    def __init__(self, opt):
        ...
        # xử lý mask_file
        if hasattr(opt, 'mask_file') and opt.mask_file != 'none':
            ...
            self.mask_size = 0  # tránh lỗi nếu bị truy cập
    ...
    def load_mask(self, img, index):
        ...
        if isinstance(self.opt.mask_type, int):
...
def dataloader(opt):
    ...
    if opt.isTrain:
        train_opt = opt
        train_opt.img_file = opt.img_file  # folder train

        train_dataset = CreateDataset(train_opt)
        train_loader = data.DataLoader(train_dataset, batch_size=opt.batchSize, shuffle=not opt.no_shuffle, num_workers=int(opt.nThreads))

        # Tạo dataset val
        val_opt = opt
        val_opt.img_file = opt.val_img_file  # folder val (bạn thêm tham số val_img_file khi gọi)

        val_dataset = CreateDataset(val_opt)
        val_loader = data.DataLoader(val_dataset, batch_size=opt.batchSize, shuffle=False, num_workers=int(opt.nThreads))
        ...
        loader = data.DataLoader(dataset, batch_size=opt.batchSize, shuffle=False, num_workers=int(opt.nThreads))
        return loader
