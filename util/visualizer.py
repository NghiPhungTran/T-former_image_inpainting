...
class Visualizer():
    ...
    def plot_epoch_losses(self, epoch, losses_dict):
        """Hiển thị biểu đồ Train Loss và Val Loss theo từng epoch"""
        if not hasattr(self, 'epoch_plot_data'):
            self.epoch_plot_data = {'X': [], 'Y': [], 'legend': list(losses_dict.keys())}

        # Kiểm tra thiếu key
        missing = set(self.epoch_plot_data['legend']) - set(losses_dict.keys())
        if missing:
            print(f"⚠️ Missing keys in losses_dict at epoch {epoch}: {missing}")
            return

        # Kiểm tra NaN
        for k in self.epoch_plot_data['legend']:
            v = losses_dict[k]
            if v is None or np.isnan(v):
                print(f"⚠️ Invalid value for {k}: {v}")
                return

        self.epoch_plot_data['X'].append(epoch)
        self.epoch_plot_data['Y'].append([losses_dict[k] for k in self.epoch_plot_data['legend']])

        self.vis.line(
            X=np.stack([np.array(self.epoch_plot_data['X'])] * len(self.epoch_plot_data['legend']), 1),
            Y=np.array(self.epoch_plot_data['Y']),
            opts={
                'title': 'Train vs Val Loss (per Epoch)',
                'legend': self.epoch_plot_data['legend'],
                'xlabel': 'Epoch',
                'ylabel': 'Loss'
            },
